...
def checkin():
    ...
    last_checkin = db_fetch_one("SELECT recovery_last_checkin FROM users WHERE user_id = %s", (request.user_id,))
    last = last_checkin[0] if last_checkin else None
    if last == today:
    ...
    if status == "yes":
        if last and (today - last).days == 1:
            streak = db_fetch_one("SELECT recovery_streak FROM users WHERE user_id = %s", (request.user_id,))[0] + 1
        else:
            streak = 1
    else:
        streak = 0
...
@require_auth
def daily_verse():
    # For simplicity, return a random verse from a pre‑defined list
    verses = [
...
def send_message():
    ...
    mentorship = db_fetch_one("SELECT id FROM recovery_mentorships WHERE (user_id = %s AND mentor_id = %s) OR (user_id = %s AND mentor_id = %s)", (request.user_id, receiver_id, receiver_id, request.user_id))
    if not mentorship:
...
def get_sessions():
    ...
    sessions = []
    for r in rows:
        sessions.append({"id": r[0], "title": r[1], "description": r[2], "time": str(r[3]), "room": r[4]})
...
def check_upcoming_sessions():
    ...
    for row in rows:
        # Notify all participants? For MVP, notify all users (or fetch participants)
